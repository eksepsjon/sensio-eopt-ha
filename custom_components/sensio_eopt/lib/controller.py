...
class SensioEoptController:
    ...
    async def connect(self) -> ControllerInfo:
        ...
        try:
            ...
            requestable = {n: i for n, i in id_cache.items()
                           if n.startswith(("D_", "M_"))}
            if requestable:
                _LOGGER.debug(
                    "Requesting state for %d known objects via d_obj", len(requestable)
                )
                for numeric_id in requestable.values():
                    await self._send(f"d_obj {numeric_id}")
        except Exception as exc:
            _LOGGER.debug("Could not load id cache: %s", exc)
                ...
                for line in msgs:
                    ...
                    if "x_bm_st ACK_DIR" in line or "PANEL_BRIGHTNESS" in line:
                        try:
                            ...
                            pass
                    # Harvest name->id pairs from RSN/SSN events
                    m = _RSN_ID_RE.match(line)
                    if m:
